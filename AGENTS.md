...
- **Two Claude profiles, one config policy: `claude/.claude/settings.json` (personal) and `claude-moatlab/.claude-moatlab/settings.json` (business) must stay IDENTICAL except for `effortLevel`** (personal `high`, moatlab `xhigh`) — that one key is the only intentional divergence. Every other change — `hooks`, `model`, `env`, `permissions`, `enabledPlugins`, `autoMode`, anything — must be applied to **both** files. The two are separate accounts but share all account-independent settings; there is no auto-sync, so edits are mirrored by hand. Verify with a structural diff (`python3 -c` flatten-and-compare) rather than eyeballing, since Claude's own settings-writer reorders keys and a textual diff is noisy.
